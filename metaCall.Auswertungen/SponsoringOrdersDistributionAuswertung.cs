...
{
    [ModulIndex(3)]
    public partial class SponsoringOrdersDistributionAuswertung : UserControl, IModulMainControl
    {
        private MetaCallPrincipal principal;
        private List<SponsoringOrdersDistribution> results;
        ...
        private class Configuration : ModulConfigBase
        {
            ...
            {
                return new StartUpMenuItem("Auftragsverteilung (h)", "Auswertungen");
            }
        ...
        private void Reload()
        {
            ...
            if (node != null)
            {
                ...
                {
                    CenterInfo centerInfo = ((CenterTreeNode)node).Center;
                    results = MetaCall.Business.CallJobs.GetSponsoringOrdersDistribution(centerInfo.CenterId,null,null,null,from, to); 
                    filterText.AppendFormat("Center : {0}", centerInfo.Bezeichnung);
                }
                else if (node.GetType() == typeof(TeamInfoTreeNode))
                {
                    TeamInfo teamInfo = ((TeamInfoTreeNode)node).Team;
                    results = MetaCall.Business.CallJobs.GetSponsoringOrdersDistribution(null, teamInfo.TeamId, null, null, from, to);
                    filterText.AppendFormat("Team : {0}", teamInfo.Bezeichnung);
                }
                else if (node.GetType() == typeof(UserInfoTreeNode))
                {
                    UserInfo userInfo = ((UserInfoTreeNode)node).User;
                    results = MetaCall.Business.CallJobs.GetSponsoringOrdersDistribution(null, null, userInfo.UserId, null, from, to);
                    filterText.AppendFormat("Agent : {0}", userInfo.DisplayName);

                }
                else if (node.GetType() == typeof(ProjectInfoTreeNode))
                {
                    ProjectInfo projectInfo = ((ProjectInfoTreeNode)node).ProjectInfo;
                    results = MetaCall.Business.CallJobs.GetSponsoringOrdersDistribution(null, null, null, projectInfo.ProjectId, from, to);
                    filterText.AppendFormat("Projekt : {0}", projectInfo.Bezeichnung);
           ...
           // report = new ReportDocument();
            ...
            report.Load(@"SponsoringOrdersDistribution.rpt", CrystalDecisions.Shared.OpenReportMethod.OpenReportByTempCopy);
