...
{
    [ModulIndex(99)]
    public partial class Arbeitszeitverwaltung : UserControl, IModulMainControl
    {
        private MetaCallPrincipal principal;

        UserWorkTimeAdditionInfoView uwtaiView;

        public Arbeitszeitverwaltung()
        {
            InitializeComponent();
        ...
        private class Configuration : ModulConfigBase
        {
            ...
            {
                return new StartUpMenuItem("Arbeitszeiten", "Arbeitszeit");
            }
        ...
        private void Arbeitszeitverwaltung_Load(object sender, EventArgs e)
        {
            if (DesignMode)
                return;
        ...
        private void Reload()
        {
            this.uwtaiView = GetUserWorkTimeInfoViewControl();

            this.panelWorkTimeInfo.SuspendLayout();
            this.panelWorkTimeInfo.Controls.Clear();

            if (this.uwtaiView != null)
            {
                this.uwtaiView.Dock = DockStyle.Fill;
                this.panelWorkTimeInfo.Controls.Add(uwtaiView);
            }
            this.panelWorkTimeInfo.ResumeLayout();

        }

        private UserWorkTimeAdditionInfoView GetUserWorkTimeInfoViewControl()
        {
            UserWorkTimeAdditionInfoView uwtaiViewTMP = new UserWorkTimeAdditionInfoView(); ;
            ...
            else if (node.GetType() == typeof(CenterTreeNode))
            {
                ...
                uwtaiView = (UserInfoView)Activator.CreateInstance(typeof(UserInfoView), new Object[] { center });
                */
                uwtaiViewTMP = null;
            }
            else if (node.GetType() == typeof(TeamInfoTreeNode))
            {
                ...
                uwtaiView = (UserInfoView)Activator.CreateInstance(typeof(UserInfoView), new Object[] { team });
                */
                uwtaiViewTMP = null;
            }
            else if (node.GetType() == typeof(UsersWithOutCenterTreeNode))
            {
                ...
                uwtaiView = (UserInfoView)Activator.CreateInstance(typeof(UserInfoView), new Object[] { uTyp });
                */
                uwtaiViewTMP = null;
            }
            ...
            else if (node.GetType() == typeof(UserInfoTreeNode))
            {
                ...
                User user = MetaCall.Business.Users.GetUser(((UserInfoTreeNode)node).User);
                uwtaiViewTMP = (UserWorkTimeAdditionInfoView)Activator.CreateInstance(typeof(UserWorkTimeAdditionInfoView), new Object[] { user, from, to});
            }
            else
            {
                uwtaiViewTMP = null;
        ...
        {
            if (this.principal.IsInRole(MetaCallPrincipal.AdminRoleName) || MetaCall.Business.Users.CurrentUser.WorkingTimeEditPermit == true)
                return true;
            else
                return false;
